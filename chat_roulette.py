...
def _get_msg_from_app():
	msg = raw_input("Type a message: ")
	return msg	 


def run():  
	print "Welcome to Chat Roulette with Mei Messaging!"
	print "Type 'exit' to exit!"
	
	while True:
            msg = self._get_msg_from_app()
	    if msg.lower() == "exit":
		break
	    start = time.time()
	    users_traversed = self._get_cr_response(msg)
	    end = time.time()
	    total = end - start
	    print "USERS TRAVERSED: " + str(users_traversed)
	    print "TOTAL TIME /secs: " + str(total) + "\n" 
"""
